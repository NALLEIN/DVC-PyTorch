...
class Extractor_net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, out_channel_mv, 3, stride=2, padding=1)
        torch.nn.init.xavier_normal_(self.conv1.weight.data, (math.sqrt(2 * (3 + out_channel_mv) / (6))))
        torch.nn.init.constant_(self.conv1.bias.data, 0.01)
        self.prelu1 = nn.LeakyReLU(inplace=True)
        self.conv2 = nn.Conv2d(out_channel_mv, out_channel_mv, 3, stride=2, padding=1)
        torch.nn.init.xavier_normal_(self.conv2.weight.data, math.sqrt(2))
        torch.nn.init.constant_(self.conv2.bias.data, 0.01)
        self.prelu2 = nn.LeakyReLU(inplace=True)
        self.conv3 = nn.Conv2d(out_channel_mv, out_channel_mv, 3, stride=2, padding=1)
        torch.nn.init.xavier_normal_(self.conv3.weight.data, math.sqrt(2))
        torch.nn.init.constant_(self.conv3.bias.data, 0.01)
        self.prelu3 = nn.LeakyReLU(inplace=True)
        self.conv4 = nn.Conv2d(out_channel_mv, out_channel_mv, 3, stride=2, padding=1)
        torch.nn.init.xavier_normal_(self.conv4.weight.data, math.sqrt(2))
        torch.nn.init.constant_(self.conv4.bias.data, 0.01)
...
class Attention_net(nn.Module):
    def __init__(self):
        ...
        torch.nn.init.constant_(self.conv1.bias.data, 0.01)
        self.relu1 = nn.ELU()
        self.conv2 = nn.Conv2d(out_channel_mv // 2, out_channel_mv, 3, 1, 1)
        torch.nn.init.xavier_normal_(self.conv2.weight.data, math.sqrt(2))
        torch.nn.init.constant_(self.conv2.bias.data, 0.01)
...
class Generator_net(nn.Module):
    def __init__(self):
        ...
        self.prelu2 = nn.LeakyReLU(inplace=True)
        #self.deconv3 = nn.ConvTranspose2d(out_channel_mv, out_channel_mv, 3, stride=2, padding=1)
